          ...
          <div class="bar-chart">
            <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  class="bar"
                  :style="{ height: barHeights[month.month] + 'px' }"
...
export default {
  ...
  setup() {
    ...
    // Pre-compute bar heights once per data load (O(n) not O(n^2))
    const barHeights = computed(() => {
      const maxRevenue = Math.max(...monthlyData.value.map(m => m.revenue), 0)
      if (maxRevenue === 0) return {}
      return Object.fromEntries(
        monthlyData.value.map(m => [m.month, (m.revenue / maxRevenue) * 200])
      )
    })
